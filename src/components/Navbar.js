    ...
    <header className="relative z-10 shadow-lg bg-amber-500">
      <div className="container flex items-center justify-between">
        <Link className="animation hover:scale-105" to="./home-page.html">
          <img className="w-56 h-auto md:w-64" src="/cover.png" alt="MRHYA Logo" />
        </Link>

        <nav>
          <ul className="hidden gap-14 md:flex">
            <li className="list-item active-list-item">
              <Link to="/">Home</Link>
            </li>

            <li className="relative group list-item">
      ...
      <nav
        className={`${isNavbar && 'active-navbar'} absolute z-50 w-full py-6 opacity-0 pointer-events-none md:hidden animation -top-40 bg-amber-500`}
      >
        <ul className="flex flex-col items-center w-full gap-8">
          <li className="mobile-list-item active-mobile-list-item">
            <Link to="/home">Home</Link>
          </li>
          <li className="mobile-list-item" onClick={() => setIsDropdown(!isDropdown)}>
            Clothes
            <svg
              width="36"
              height="36"
              ...
              className={`${isDropdown && 'arrow-animation'} inline-block fill-amber-50 animation hover:fill-amber-900`}
            >
            ...
            <ul
              className={`${isDropdown && 'active-mobile-dropdown'} absolute mt-2 pl-2 space-y-3 opacity-0 pointer-events-none animation border-t-[1px]`}
            >
              <li className="dropdown-item">
                <Link to="">Man</Link>
              </li>
